...
export const PATCH: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      SET ${setClauses.join(", ")}
      WHERE id = $${idx}
      RETURNING id, job_number, year, description,
                customer_id, customer_name_raw, contract_type,
                tax_status, tax_exemption_type,
                general_contractor, project_manager,
...
async function handleList(url: URL): Promise<Response> {
  ...
  const queryText = `
    SELECT jm.id, jm.job_number, jm.year, jm.description,
           jm.customer_id, jm.customer_name_raw, jm.contract_type,
           jm.tax_status, jm.tax_exemption_type,
           jm.general_contractor, jm.project_manager,
